...
AFRAME.registerComponent('model-viewer', {
    ...
    init: function () {
      ...
      this.submitButtonPosResetClicked = this.submitButtonPosResetClicked.bind(this);
    ...
    initUploadInput: function () {
      ...
      var inputE2 = this.inputE2 = document.createElement('input');
      var inputE3 = this.inputE3 = document.createElement('input');
      var submitButtonEl = this.submitButtonEl = document.createElement('button');
      var submitButtonPosReset = this.submitButtonPosReset = document.createElement('button');
      ...
      var css =
        ...
        '.a-upload-model-input {color: #666; vertical-align: middle; padding: 0px 10px 0 10px; text-transform: uppercase; border: 0; width: 20%; height: 100%; border-radius: 10px; margin-right: 10px}' +
      ...
      submitButtonEl.innerHTML = 'Start';
      submitButtonEl.addEventListener('click', this.submitURLButtonClicked);

      submitButtonPosReset.classList.add('a-upload-model-button');
      submitButtonPosReset.innerHTML = 'Reset pozycji';
      submitButtonPosReset.addEventListener('click', this.submitButtonPosResetClicked);
      ...
      inputE2.value = inputDefaultValue;

      inputE2.classList.add('a-upload-model-input');
      inputE2.onfocus = function () {
        if (this.value !== inputDefaultValue) { return; }
        this.value = '';
      };
      inputE2.onblur = function () {
        if (this.value) { return; }
        this.value = inputDefaultValue;
      };
      ...
      inputE3.classList.add('a-upload-model-input');
      inputE3.onfocus = function () {
        if (this.value !== inputDefaultValue) { return; }
        this.value = '';
      };
      inputE3.onblur = function () {
        if (this.value) { return; }
        this.value = inputDefaultValue;
      };
  
      inputE3.value = inputDefaultValue;
  
      uploadContainerEl.appendChild(inputEl);
      uploadContainerEl.appendChild(inputE2);
      uploadContainerEl.appendChild(inputE3);
      uploadContainerEl.appendChild(submitButtonEl);
      uploadContainerEl.appendChild(submitButtonPosReset);
    ...
    submitURLButtonClicked: function (evt) {
      x0 = parseFloat(this.inputEl.value);
      v0 = parseFloat(this.inputE2.value);
      a = parseFloat(this.inputE3.value);
      this.modelEl.object3D.position.x = x0;
      aktCzas = 0;
      moveStart = true;
      //this.modelEl.object3D.position.x += 0.5;
    },

    submitButtonPosResetClicked: function (evt) {
      moveStart = false;
      this.modelEl.object3D.position.x = -center.x;
      this.modelEl.object3D.position.y = -center.y;
    ...
    initEntities: function () {
      ...
      modelEl = this.modelEl = document.createElement('a-entity');
